...
```{r}
# Manually create a prediction grid
newdata <- expand.grid(
  agep = seq(20, 80, by = 2),
  hincp = c(20000, 50000, 80000, 110000)
)

# Predict using the parsnip model (automatically uses logit link)
newdata$pred_prob <- predict(model, newdata, type = "prob")$.pred_FullAccess

# If you want standard errors / confidence intervals,
# use the underlying glm object
model_glm <- model$fit

# Add confidence intervals manually using predict.glm
pred <- predict(model_glm, newdata = newdata, type = "link", se.fit = TRUE)
newdata$fit_link <- pred$fit
newdata$se_link <- pred$se.fit

# Convert link scale to probability scale and add 95% CI
newdata <- newdata %>%
  mutate(
